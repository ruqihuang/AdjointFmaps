function [A] = angles_of_triangles(X, T)
    % Computes for each triangle the 3 angles among its edges.
    % Input:
    %   option 1:   [A] = angles_of_triangles(X, T)
    %
    %               X   - (num_of_vertices x 3) 3D coordinates of
    %                     the mesh vertices.
    %               T   - (num_of_triangles x 3) T[i] are the 3 indices
    %                     corresponding to the 3 vertices of the i-th
    %                     triangle. The indexing is based on -X-.
    %
    ...
    L1 = sqrt(sum((X(T(:,2),:) - X(T(:,3),:)).^2, 2));  
    L2 = sqrt(sum((X(T(:,1),:) - X(T(:,3),:)).^2, 2));
    L3 = sqrt(sum((X(T(:,1),:) - X(T(:,2),:)).^2, 2)); 
